...
def cigo_api_sample():
    cc = CigoConnect(debug=True, account_id="ACCOUNT_ID", auth_key="AUTH_KEY")
    ...
    job = Job(
        date="2021-06-22",
        customer_first_name="Abdullah",
        customer_last_name="Coded",
        phone_number="+96590000000",
        address="building 3, street 2, block 1, Shuwaikh, Kuwait",
    )
    ...
    action_response = cc.create_job_action(
        job_id, JobAction(ref_id="13", action_type=JobActionType.delivery.value, description="delivery of grocery")
    )
